...
- **Real Player IP Injection (`PROXY TCP4`)**: Dedicated `tcp-proxy` and `minecraft-proxy` stream modes that inject HAProxy PROXY Protocol v1 headers at the start of TCP streams, enabling Minecraft servers (`proxy-protocol: true`) to log real player IP addresses instead of localhost (`127.0.0.1`).
