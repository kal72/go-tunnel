...
### Added
- **Traffic Control & Rate Limiting**: Added configurable token-bucket rate limiting ala Ngrok / Cloudflare (`internal/shared/ratelimit`). Protects active tunnel hostnames from excessive HTTP requests and DDoS surges before multiplexing TCP streams. Includes global ON/OFF toggle, configurable request rate (`req/s`) and burst capacity, and an Admin Bypass switch (`admin_allowed`) controlled via System Settings (`/api/settings` and Web UI). Designed purely in-memory without audit logging per user constraint.
